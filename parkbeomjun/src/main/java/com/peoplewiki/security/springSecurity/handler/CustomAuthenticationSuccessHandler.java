package com.peoplewiki.security.springSecurity.handler;
...
public class CustomAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication)
		...
		// 로그인 성공시 그 동안 실패해서 세션에 저장해두었던 인증 실패 관려 정보를 삭제 한다
		clearAuthenticationAttributes(request);

		// 1. security 로그인 폼에 설정해 놓은 loginRedirect 값을 가져온다 그 값으로 리다이렉트 함
		// 2. 세션에 설정해 놓은 loginRedirect 값을 가져온다 그 값으로 리다이렉트 함
		// 3. 이전 페이지로 이동(리퍼러)
		// 4. security 설정에 정의해 놓은 defaultUrl 값을 가져온다 그 값으로 리다이렉트 함
