...
/**
 * form 태그에 있는 input, textarea, select, button 의 
 * 속성을 분석하여 ArrayList에 담는다. 
 */
public final class FormFields {
	private HashMap map=new HashMap();
	private ArrayList list=new ArrayList();
	private Map conflictList = new HashMap();
	...
	private void add(FormField formField) {
		boolean isAllowedConflict = 
			formField.getFormControlType().equals(FormControlType.CHECKBOX)
			|| formField.getFormControlType().equals(FormControlType.RADIO); 
		
		if ( !isAllowedConflict && map.containsKey(formField.getName()) ) {
			List currentConflictField = null;
			if ( conflictList.containsKey(formField.getName()) ) {	// 기존에 충돌(중복)이 발생한 키가 이미 존재할 경우 
				currentConflictField = (List)conflictList.get(formField.getName()) ;
			} else {
				currentConflictField = new ArrayList();
				currentConflictField.add(map.get(formField.getName()));
			}
			currentConflictField.add(formField);
			conflictList.put(formField.getName().toLowerCase(), currentConflictField);
		} else {
	...
	/**
	 * input 태그 분석 (input 완료) 
	 * @param segment
	 */
	private void loadInputControls(Segment segment) {
		// input 태그의 경우 EndTag가 필요 없으므로 
		// findAllStartTags를 사용 
	...
	private void loadSelectControls(Segment segment) {
		...
		if (optionTags==null){	// Option 이 없을 경우.
			StartTag selectTag=selectElement.getStartTag();
			Attribute nameAttribute=selectTag.getAttributes().get("name");
			//registerPredefinedValueField( nameAttribute.getValue(), selectTag, null, null, selectTag.getFormControlType(), null ); jsp빌드 오류로 아래의 코드로 수정함.
			registerPredefinedValueField( nameAttribute.getValue(), selectElement, null, null, selectTag.getFormControlType(), null );
			return;
		}		
...
//			System.out.println("111 : " + optionTag.getFollowingTextSegment().getSourceTextNoWhitespace());
			...
			registerPredefinedValueField(name,selectElement, null, value,formControlType, null);
		}
	}
...
//			registerPredefinedValueField(name, position, predefinedValue, formControlType);
	...
	private void registerPredefinedValueField(String name, Segment segment, String size, String predefinedValue, FormControlType formControlType, List attributeList) {
			...
			|| formControlType.equals(FormControlType.CHECKBOX) ) 
		{
			...
			if ( formControlType.equals(FormControlType.TEXT) || formControlType.equals(FormControlType.HIDDEN) ) {
				add(formField=new FormField(name,segment,formControlType, attributeList));
			} else {
...
//		}
	...
	public List getConflictList(String name) {
		return (List)conflictList.get(name.toLowerCase());
	}
